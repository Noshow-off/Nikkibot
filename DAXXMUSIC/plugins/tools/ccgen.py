...
async def generate_cc(client, message):
    if len(message.text.split()) == 2:
        ...
        await message.reply("𝙸𝙽𝚅𝙰𝙻𝙸𝙳 𝙵𝙾𝚁𝙼𝙰𝚃 ⚠", parse_mode=enums.ParseMode.HTML, disable_web_page_preview=True)
        return

    if amount > 20000:
        await message.reply("𝙶𝙴𝙽 𝙻𝙸𝙼𝙸𝚃 20000 ⚠", parse_mode=enums.ParseMode.HTML, disable_web_page_preview=True)
        return
    ...
    loading_message = await message.reply("𝙶𝙴𝚃𝚃𝙸𝙽𝙶 𝙲𝙲", parse_mode=enums.ParseMode.HTML, disable_web_page_preview=True)
...
[ϟ] 
[ϟ] 𝙰𝚖𝚘𝚞𝚗𝚝: <code>{amount}</code>
[ϟ] 𝙴𝚡𝚝𝚛𝚊𝙿: <code>{text}</code>

[ϟ] 𝙶𝚎𝚗 𝙱𝚢: <a href="tg://user?id={message.from_user.id}">{message.from_user.first_name}</a>
{bin_info}
"""
        try:
